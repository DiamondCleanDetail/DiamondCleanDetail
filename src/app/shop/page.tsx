...
function ProductGrid({ items }: { items: typeof products }) {
  return (
    <StaggerGrid className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5">
      {items.map((product) => (
        <StaggerItem key={product.slug}>
          <div className="card-lift h-full bg-surface border border-border rounded-xl p-4 sm:p-5 flex flex-col">
            <div className="h-24 sm:h-32 rounded-lg bg-surface-2 mb-4" />
            <h2 className="font-medium text-sm sm:text-base">{product.name}</h2>
            <p className="text-xs sm:text-sm text-muted mt-2 flex-1">
              {product.description}
            </p>
...
export default function ShopPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-10 sm:py-16">
      ...
      <FadeIn>
        <h2 className="text-lg sm:text-xl font-semibold mb-4">Detailing Supplies</h2>
      </FadeIn>
      <ProductGrid items={supplies} />

      <FadeIn>
        <h2 className="text-lg sm:text-xl font-semibold mb-4 mt-10 sm:mt-12">Gift Cards</h2>
      </FadeIn>
